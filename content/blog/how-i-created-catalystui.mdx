...
But building a great product requires more than just coding or designing and that's what I'm learning nowadays.

I am one of those people who believe in building great products at great speed.

Everytime I build a product, I felt overwhelmed by all the UI/UX options reagarding layout, spacing, typography, and color.

Hence I created a minimal component library for myself which follows a specific set of rules to avoid being overwhelmed with all the options.

Introducing: **Catalyst UI** - _A React comomponents library for faster product prototyping_.
...
- Avatar.Image: The element which takes image url
- Avatar.Fallback: The element which render name initials if image url is not given or can't be fetched
...
This renders a completely unstyled Avatar, one which doesn't even look like an avatar. So that we can style it however we want. And that's not a bug but a feature of RadixUI Primitives.

We'll create three different components for each part of avatar.
...
Similarly other components are also built with some API differences. Radix doesn't have primitives for everything so for other things we can use Slot utility from Radix to get its accessibility benefits into our custom primitives.
